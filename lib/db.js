...
var DB = function(settings) {
    ...
    self.connect  = function(){
        ...
        var modulePath = path.join(path.dirname(require.main.filename), self.settings.models || DEFAULT_MODELS_PATH);
        ...
        async.waterfall([
            ...
            function(next) {
                fs.stat(modulePath, function(err) {
                    if (err) {
                        console.log(`[WARNING] can't access models path ${modulePath}. Starting with no custom model support`);
                    ...
                    async.waterfall([
                        ...
                        function(table_next_step) {
                            if (self.customModels) {
                                var customFile = path.join(modulePath, `${table_name}.js`);
